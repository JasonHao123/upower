...
wx.ready(function(){
	...
	wx.onMenuShareTimeline({
	    title: '${profile.nickname}社交圈的地区分析', // 分享标题
	...
	wx.onMenuShareAppMessage({
		title:'${profile.nickname}社交圈的地区分析',
...
$( document ).on( "pagecreate", "#myPage", function() {
    ...
    var  ageData = [
                    <c:forEach items="${result.data}" var="item">
                    <c:if test="${item.series=='PROVINCE'}">
                    { key:'${item.key}', value:${item.value}},
                    </c:if>
              		</c:forEach>
                    ];
...
<h2>${profile.nickname}社交圈的地区分析</h2>
