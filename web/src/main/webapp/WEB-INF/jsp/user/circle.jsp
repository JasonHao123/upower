	...
	$.getJSON("<c:url value="/user/circle2.do" />?page="+pageNo,function(result){
	    if(result.length>0) {
		    $.each(result, function(i, message){
			      $("#messages").append("<li><a data-ajax='false' href='<c:url value="/user/profile.do" />?id="+message.id+"' ><h2>"+message.nickname +"("+message.distance +"度)</h2>   <p>"+message.nickname+"</p> <p class=\"ui-li-aside\">"+message.nickname +"</p> </a></li>");
			    });
